...
def modelate_with_different_connectivity(window_size, label, connectivity_number_total, G, conn_empty_values):
    ...
    for i in range(connectivity_number_total):
        ...
        graphs, _ = G.modelate(window_size = window_size, connectivity = conn, bands = bands, threshold = 0.55)                         #<--------------- THRESHOLD
...
def avg_density(graphs):
    ...
    print('STD:', round(np.std(density_values),4))
...
def avg_edges(graphs):
    ...
    for graph in graphs:
        ...
        count_total.append(graph.number_of_edges())
    
    avg = round(count / len(graphs), 1)
    print('Avg Number of Edges:', avg)
    print('STD:', round(np.std(count_total),3))
...
def avg_clustering_graphs(graphs):
    ...
    for graph in graphs:
        ...
        clustering_std.append(clustering)
        
    avg = clustering_total / len(graphs)
    print('Avg: ', round(avg,4))
    print('STD:', round(np.std(clustering_std),3))
...
def node_degree(graphs):
    ...
    for i in range(len(nodes)):
        print('Nodo: ', nodes[i], ' - ', avg_degrees[i], '\tSTD:', round(np.std(matrix[i]),2))
...
def degree_centrality(graphs):
    ...
    for graph in graphs:
        ...
        count = 0
        for val in degree_centrality.values():
            matrix[count].append(val)
            count+=1
    ...
    for k,v in avg.items():
        print('Nodo: ', k, '- ', round(v,4), '\tSTD:', round(np.std(matrix[i]),4))
        i+=1
...
def betweenness_centrality(graphs):
    ...
    for graph in graphs:
        ...
        count = 0
        for val in bw_centrality.values():
            matrix[count].append(val)
            count+=1
